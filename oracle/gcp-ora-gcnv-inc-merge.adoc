...
---
= TR-5007: Optimieren Sie Oracle VLDB Backup & Recovery mit inkrementeller Zusammenführung auf Google Cloud NetApp Volumes
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Die Wiederherstellung einer sehr großen Datenbank (VLDB) in Oracle mit dem Sicherungstool Oracle Recovery Manager (RMAN) kann eine äußerst anspruchsvolle Aufgabe sein.  Der Datenbankwiederherstellungsprozess von Sicherungsmedien im Falle eines Fehlers kann zeitaufwändig sein, die Datenbankwiederherstellung verzögern und möglicherweise Ihr Service Level Agreement (SLA) erheblich beeinträchtigen.  Für in der Cloud bereitgestellte Oracle VLDBs wird die Situation sogar noch schlimmer, da das RMAN-Backup aus Kostengründen im Allgemeinen in den Objektspeicher gestreamt wird.  Daher ist die Wiederherstellung und Wiedergewinnung einer VLDB nicht praktikabel, wenn ein SLA unerlässlich ist.  Ab Version 10g hat Oracle eine RMAN-Funktion eingeführt, mit der Benutzer stufenweise Image-Kopien der Oracle-Datenbankdatendateien auf zusätzlichem Festplattenspeicher auf dem DB-Server-Host erstellen können.  Diese Image-Kopien können mithilfe von RMAN in benutzerdefinierten Intervallen inkrementell aktualisiert werden.  Im Falle eines Fehlers kann der Datenbankadministrator (DBA) die Oracle-Datenbank schnell vom ausgefallenen Medium auf die Image-Kopie umstellen, sodass keine vollständige Wiederherstellung des Datenbankmediums erforderlich ist.  Das Ergebnis ist ein deutlich verbessertes SLA, allerdings auf Kosten einer Verdoppelung des erforderlichen Datenbankspeichers.

Wenn Sie Wert auf SLA für Ihre VLDB legen und erwägen, die Oracle-Datenbank in eine öffentliche Cloud wie Google Cloud Platform (GCP) zu verschieben, können Sie eine ähnliche Datenbankschutzstruktur mit Ressourcen wie Google Cloud NetApp Volumes (GCNV) zum Staging Ihrer Standby-Datenbank-Imagekopie einrichten.  GCNV ist ein vollständig verwalteter, leistungsstarker Dateispeicherdienst von Google, der die Migration und Ausführung anspruchsvoller Unternehmensanwendungen wie Oracle-Datenbanken in Google Cloud einfacher, sicherer und kostengünstiger macht.  Die verschiedenen Service-Levels ermöglichen die flexible Kontrolle der Speicherkosten und erreichen gleichzeitig einen RPO von nahezu null und einen überschaubaren RTO.  In dieser Dokumentation zeigen wir, wie Sie ein NFS-Dateisystem aus einem GCNV-Speicherpool bereitstellen und exportieren, um es auf einem Oracle-Datenbankserver bereitzustellen und so eine Standby-Datenbankkopie für eine schnelle Wiederherstellung im Falle eines primären Speicherausfalls bereitzustellen.

Diese Lösung ist für die folgenden Anwendungsfälle geeignet:

* Imagekopie-Sicherung der Oracle-Datenbankdatendateien über RMAN auf ein serverinternes NFS-Dateisystem in einem Google Cloud NetApp Volumes -Speicherpool.
* Schnelle Wiederherstellung einer Oracle VLDB im Falle eines Fehlers auf derselben oder einer Standby-VM der Google Compute Engine.
...
Das Testen und Validieren dieser Lösung wurde in Google Cloud NetApp Volumes -Speicherpools und Google Compute Engine-VMs durchgeführt, die möglicherweise nicht der endgültigen Bereitstellungsumgebung entsprechen. Weitere Informationen finden Sie im Abschnitt <<Wichtige Faktoren für die Bereitstellungsüberlegungen>> .
...
image:gcp-ora-gcnv-inc-merge-architecture.png["Dieses Bild bietet eine detaillierte Darstellung der inkrementellen Zusammenführungsimplementierung von Oracle VLDB in der Google Cloud mit GCNV."]
...
| Google Compute Engine VM für DB-Server | N1 (4 vcpus, 15 GiB Speicher) | 2 VMs, eine als primärer DB-Server und die andere als Standby- oder Klon-DB-Server 
...
* *GCNV-Speicherpool-Serviceebene.*  Der GCNV-Speicherpool bietet vier Servicelevel: Standard, Premium, Extreme und Flex.  Standardmäßig wird für ein in einem Speicherpool erstelltes Volume eine automatische QoS-Funktion angewendet, die den Durchsatz des Volumes basierend auf seiner Größe und dem Servicelevel beschränkt.  Die einzige Ausnahme besteht darin, dass aus dem Flex-Speicherpool zugewiesene Volumes den für den Speicherpool verfügbaren Durchsatz gemeinsam nutzen.
* *Speicherlimit für Oracle VLDB-Backups im Server für inkrementelle RMAN-Zusammenführung.*  In unserem Test und unserer Validierung wird ein NFS-Volume für inkrementelle Oracle-Backups und -Zusammenführungen aus einem einzelnen GCNV-Speicherpool mit Standard-Servicelevel zugewiesen, der eine Kapazitätsgrenze von 200 TiB hat.  Für die Bereitstellung über diesen Schwellenwerten können mehrere GCNV-Speicherpools parallel mit mehreren NFS-Mount-Punkten implementiert werden, um eine höhere Kapazität bereitzustellen.
* *Oracle-Wiederherstellung durch inkrementelles Zusammenführen mit RMAN.*  Die inkrementelle Sicherung und Zusammenführung von RMAN im Server wird im Allgemeinen in einer benutzerdefinierten Häufigkeit basierend auf Ihren RTO- und RPO-Zielen ausgeführt.  Bei einem vollständigen Verlust des primären Datenbankdateispeichers und/oder der archivierten Protokolle kann es zu Datenverlust kommen.  Die Oracle-Datenbank kann bis zur letzten verfügbaren inkrementellen Sicherung aus der Imagekopie der Datenbanksicherung im Server wiederhergestellt werden.  Um Datenverluste zu minimieren, muss der Oracle-Flash-Wiederherstellungsbereich auf dem GCNV-NFS-Backup-Mount-Punkt eingerichtet werden. Archivierte Protokolle werden zusammen mit einer Datenbank-Image-Kopie auf demselben NFS-Mount gesichert, um im Falle eines Fehlers einen Rollforward der Datenbank auf das letzte archivierte Protokoll durchzuführen.  Somit kann ein RPO von nahezu Null erreicht werden.
* *Oracle VLDB wird über eine NFS-Image-Sicherung im GCNV-Speicherpool ausgeführt.*  Derzeit wird die Neuzuweisung des Datenbank-Volume-Servicelevels von Standard auf Premium oder Extreme nicht unterstützt.  Die Neuzuweisung von Service-Levels von Premium auf Extreme wird jedoch unterstützt.  Um die Datenbank nach einem Fehler für einen längeren Zeitraum auf der Imagekopie auszuführen, wird empfohlen, ein Volume aus dem Premium-Speicherpool für die Sicherung der Datenbank-Imagekopie zuzuweisen.
* *Google Compute Engine-VMs.*  Beim Test und der Validierung haben wir GCE-VM-Instanzen vom Typ N1 als Oracle-Datenbankserver verwendet.  Es gibt andere Arten von GCE-VMs, die möglicherweise besser optimiert und für die Datenbankarbeitslast besser geeignet sind.  Passen Sie die Größe der VM entsprechend der Anzahl der vCPUs und der RAM-Menge an die tatsächlichen Arbeitslastanforderungen an.
* *dNFS-Konfiguration.* dNFS ist in den Oracle-Kernel integriert und steigert die Leistung von Oracle-Datenbanken deutlich, wenn Oracle auf NFS-Speicher bereitgestellt wird. dNFS ist in der Oracle-Binärdatei enthalten, aber standardmäßig nicht aktiviert.  Es sollte für jede Oracle-Datenbankbereitstellung auf NFS aktiviert werden.  Für die Bereitstellung mehrerer GCNV-Speicherpools für eine VLDB sollten dNFS-Mehrfachpfade zu verschiedenen GCNV-Speicherpools ordnungsgemäß konfiguriert werden.
...
Die Lösung geht davon aus, dass in der Google Cloud-Umgebung innerhalb einer VPC bereits eine Oracle VLDB bereitgestellt ist.  Die Oracle VLDB kann entweder auf dem GCNV-Speicher oder anderen Speicheroptionen innerhalb des Google Cloud-Ökosystems ausgeführt werden.  Der folgende Abschnitt enthält schrittweise Bereitstellungsverfahren zum Einrichten einer Oracle VLDB-Imagesicherung und inkrementellen Zusammenführung über RMAN in ein NFS-Dateisystem, das aus einem GCNV-Speicherpool gemountet ist.  Der primäre Speicher der Oracle VLDB wird ebenfalls auf einem anderen GCNV-Speicherpool gehostet.
...
. Befolgen Sie in der Google Cloud-Portalkonsole die Dokumentationlink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool["Erstellen eines Speicherpools"^] um einen GCNV-Speicherpool mit Standarddienst zu erstellen, wie unten gezeigt.
+
image:gcp-ora-gcnv-inc-merge-pool-001.png["Dieses Bild zeigt den GCNV-Speicherpool-Bildschirm von der Google Cloud-Portalkonsole"]

. Befolgen Sie in der Google Cloud-Portalkonsole die Dokumentationlink:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume["Erstellen eines Volumes"^] um ein Volume für die serverinterne Oracle VLDB-Imagesicherung zu erstellen.  In dieser Lösungsdemo wird der Oracle VLDB-Primärspeicher auch auf GCNV gehostet, wie unten gezeigt.
+
image:gcp-ora-gcnv-inc-merge-vol-002.png["Dieses Bild zeigt den GCNV-Speichervolumes-Bildschirm der Google Cloud-Portalkonsole"]
...
+
image:gcp-ora-gcnv-inc-merge-vol-003.png["Dieses Bild zeigt den GCNV-Speichervolumes-Bildschirm der Google Cloud-Portalkonsole"]

. Melden Sie sich beim primären Oracle VLDB-Server an.  Mounten Sie als Root das aus dem GCNV-Speicher exportierte NFS-Volume.  Ändern Sie nach Bedarf die IP-Adresse und den Dateipfad Ihres NFS-Servers.
+
[source, cli]
----
sudo mkdir /nfsgcnv
...
=== Einrichten der inkrementellen Zusammenführung von Oracle RMAN mit einer Image-Kopie auf GCNV für die VLDB-Sicherung auf dem Server

[%collapsible%open]
====
Die inkrementelle Zusammenführung von RMAN aktualisiert die Imagekopie der Datendateien der Staging-Datenbank kontinuierlich bei jedem inkrementellen Sicherungs-/Zusammenführungsintervall.  Die Imagekopie der Datenbanksicherung ist so aktuell wie die Häufigkeit, mit der Sie die inkrementelle Sicherung/Zusammenführung ausführen.  Berücksichtigen Sie daher die Datenbankleistung sowie Ihre RTO- und RPO-Ziele, wenn Sie die Häufigkeit inkrementeller RMAN-Sicherungen und -Zusammenführungen festlegen.

. Melden Sie sich als Oracle-Benutzer beim primären Oracle VLDB-Server an.
. Für diese Lösungsvalidierung wird die primäre Datenbank als eigenständiger Server mit Grid-Infrastruktur eingerichtet. Starten Sie die Konfiguration wie unten gezeigt neu.  Der primäre Speicher wird auf einem GCNV-Speicherpool mit Premium-Service gehostet.  Auch dies ist keine Voraussetzung und der primäre Speicher kann sich auf jeder anderen Speicheroption in der Google Cloud befinden.
+
....
...
----
. Melden Sie sich über sqlplus bei der Oracle-Datenbank an, aktivieren Sie die Blockänderungsverfolgung für eine schnellere inkrementelle Sicherung und ändern Sie den Oracle-Flash-Wiederherstellungsbereich in die GCNV-NFS-Einbindung, wenn er sich derzeit auf dem primären Speicher befindet.  Dadurch können die automatische Sicherung der RMAN-Standardsteuerdatei/SP-Datei und der archivierten Protokolle auf dem GCNV NFS-Mount gesichert werden, um die Datenbank auf das letzte verfügbare Archivprotokoll wiederherzustellen.
+
[source, cli]
----
sqlplus / as sysdba
...
....
. Listen Sie die Datenbank-Image-Kopie nach der Sicherung auf, um festzustellen, ob eine Datenbank-Image-Kopie im GCNV-NFS-Mount-Punkt erstellt wurde.
+
....
...
....
. Führen Sie das RMAN-Sicherungsskript in den gewünschten Intervallen aus einem einfachen UNIX-Shell-Skript von Crontab aus.
+
....
...
Damit ist die Einrichtung einer Oracle VLDB-Standby-Image-Kopiesicherung und -Zusammenführung auf einem serverinternen NFS-Mount aus dem GCNV-Speicher abgeschlossen.
...
....
. Fügen Sie eine Zeile in die Testtabelle ein.  Die Zeile wird nach einer letzten inkrementellen Sicherung und Zusammenführung eingefügt und ihre Daten sind nicht im Sicherungssatz enthalten.  Die nachfolgende Demonstration würde zeigen, dass die Testzeile wiederhergestellt wird, nachdem die Datenbank vollständig wiederhergestellt wurde.
+
....
...
....
. Überprüfen Sie nach der Wiederherstellung die Datenbankstruktur von sqlplus, um festzustellen, dass alle VLDB-Datendateien mit Ausnahme der Steuer-, temporären und aktuellen Protokolldateien jetzt auf das Kopieren auf das GCNV-NFS-Dateisystem umgestellt sind.
+
....
...
==== Kehren Sie die Oracle VLDB von der Sicherungskopie zurück in den primären Speicher

[%collapsible%open]
====
Nachdem der Speicherfehler im Primärspeicher behoben wurde, kann die Datenbank mit minimaler Ausfallzeit problemlos wieder auf den Primärspeicher zurückgesetzt werden.  Im Folgenden werden die Verfahren Schritt für Schritt beschrieben.

. Überprüfen Sie in RMAN, ob sich das DB-Schema derzeit im GCNV-Sicherungsspeicher befindet.
+
....
...
....
. Nachdem die primäre Datenbank auf eine Image-Kopie umgestellt wurde, ist die primäre Datenbank jetzt eine Kopie im RMAN-Datensatz.
+
....
...
....
. Löschen Sie die nun veralteten Datendateien auf dem Primärspeicher, falls vorhanden, um Speicherplatz freizugeben und/oder Datensätze zu löschen.
+
....
...
....
. Überarbeiten Sie das RMAN-Backup- und Merge-Skript, um auf den primären Speicher und in unserem Testfall auf die ASM-Datenträgergruppe +DATA in der RMAN-Kanaldefinition zu verweisen.  Führen Sie das überarbeitete Sicherungsskript aus, um eine neue Imagekopie auf dem primären Speicher zu erstellen.
+
....
...
....
. Fahren Sie die Datenbank von RMAN herunter, um zur Backup-Image-Kopie auf dem primären Speicher zurückzukehren.
+
....
...
....
. Überprüfen Sie, ob die Datenbank jetzt wieder im primären Speicher ist.
+
....
...
[%collapsible%open]
====
Im Falle eines Fehlers, bei dem der primäre Speicher und/oder der primäre DB-Server-Host verloren gehen, gehen alle Transaktionen verloren, die nicht in den archivierten Protokolldateien enthalten sind.  Allerdings ist die auf dem GCNV-NFS-Dateisystem verfügbare Kopie Ihres Oracle-Datenbank-Backup-Images praktisch.  Mithilfe der Backup-Imagekopie können Sie die primäre Datenbank schnell auf demselben primären Datenbankserver oder einem Standby-Datenbankserver (sofern verfügbar) wiederherstellen.  In diesem Abschnitt demonstrieren wir Schritt für Schritt die Verfahren zur Wiederherstellung auf dem primären DB-Server.  Die Methoden zur Wiederherstellung auf einem neuen Standby-DB-Server sind nahezu identisch. Allerdings muss die Imagekopie der Datenbanksicherung auf dem Standby-DB-Server bereitgestellt werden, auf dem derselbe Oracle-Software-Stack installiert und konfiguriert ist.
...
. Wenn die Wiederherstellung auf einem Standby-DB-Server durchgeführt wird, stellen Sie sicher, dass es sich um dasselbe Betriebssystem, dieselbe Version und denselben Betriebssystemkernel handelt wie beim primären DB-Server-Host.  Außerdem wurden die gleiche Version und die gleichen Patches von Oracle auf dem Standby-DB-Server mit der Option „Nur Software“ installiert und konfiguriert.
...
. Die Backup-Image-Kopie der Oracle-Datenbank auf dem GCNV-NFS-Dateisystem wird dann zur Wiederherstellung auf dem Standby-DB-Server bereitgestellt.  Die folgenden Verfahren veranschaulichen die Prozessdetails.
+
...
----
. Überprüfen Sie die verfügbaren archivierten Oracle-Protokolle auf dem GCNV-NFS-Mount zur Wiederherstellung und notieren Sie sich die Protokollsequenznummer der letzten Protokolldatei.  In unserem Testfall waren es 27.  Unser Wiederherstellungspunkt liegt bei Protokollsequenznummer 28.
. Wenn Sie die Wiederherstellung von einem Standby-DB-Host durchführen, setzen Sie als Oracle-Benutzer die Variable ORACLE_HOME auf die aktuelle Oracle-Installation auf dem Standby-DB-Server und ORACLE_SID auf die SID der primären Oracle-Instanz.  In diesem Test ist es NTAP.
+
....
...
....
. Erstellen Sie als Oracle-Benutzer eine generische Oracle-Init-Datei im Verzeichnis $ORACLE_HOME/dbs mit den entsprechenden konfigurierten Administratorverzeichnissen.  Am wichtigsten ist, dass Sie Oracle `flash recovery area` Zeigen Sie auf den GCNV NFS-Mountpfad, wie im primären Oracle VLDB-Server definiert.  `flash recovery area` Die Konfiguration wird im Abschnitt `Setup Oracle RMAN incremental merge to image copy on GCNV for in-server VLDB backup` .  Legen Sie die Oracle-Steuerdatei auf das GCNV NFS-Sicherungsdateisystem fest.
+
[source, cli]
----
...
. Starten Sie als Oracle-Benutzer RMAN, um die Oracle-Wiederherstellung auf dem primären oder Standby-DB-Serverhost auszuführen.  Starten Sie zunächst die Oracle-Instanz in `nomount` Zustand.
+
....
...
....
. Datenbank-ID festlegen.  Die Datenbank-ID kann aus dem Oracle-Dateinamen der Image-Kopie auf dem GCNV-NFS-Mountpunkt abgerufen werden.
+
....
...
....
. Stellen Sie die Init-Datei aus der SP-Datei im Ordner $ORACLE_HOME/dbs wieder her, um die Parameterdatei später zu aktualisieren und mit der primären VLDB abzugleichen.
+
....
...
....
. Um zum primären Speicher zurückzukehren, befolgen Sie genau die gleichen Verfahren wie im Abschnitt beschrieben `Reverse VLDB back to primary storage from backup copy` .  Bei der Wiederherstellung auf einem Standby-DB-Server muss der primäre Speicher in den Standby-DB-Server importiert werden.  Außerdem müssen Oracle-Steuerdateien und aktuelle Online-Protokolldateien vom GCNV-NFS-Mount zurück in den primären Speicher verschoben werden, was mit wenigen Minuten Ausfallzeit erledigt werden kann.
...
[%collapsible%open]
====
Das Klonen einer neuen Oracle VLDB aus der Backup-Image-Kopie auf GCNV ähnelt der zeitpunktbezogenen Wiederherstellung der primären Datenbank auf einem neuen DB-Server.  Das NetApp Solutions Engineering-Team bietet ein Automatisierungs-Toolkit, um die gesamten Prozesse für eine schnelle (Minuten) und mühelose Implementierung zu erleichtern.

Das folgende Automatisierungs-Toolkit enthält ein Playbook, mit dem Klone einer Oracle VLDB aus einer primären Datenbank-Backup-Image-Kopie erstellt oder aktualisiert werden können, die per NFS-Mount auf einem GCNV-Speicher gespeichert ist, um ein vollständiges Klon-Lebenszyklusmanagement zu ermöglichen.  Einzelheiten finden Sie in der Dokumentation:link:automation-ora-bkup-clone.html["Automatisierter Oracle VLDB-Klon-Lebenszyklus mit In-Server-Image-Backup"] .
