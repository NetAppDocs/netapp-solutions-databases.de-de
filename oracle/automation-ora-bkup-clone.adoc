...
summary: 'Die Lösung bietet ein auf Ansible basierendes Automatisierungs-Toolkit zum Einrichten, Klonen und Aktualisieren einer von Oracle geklonten Very Large Database (VLDB) aus einer Sicherungskopie eines Oracle-Datenbank-Images, die sich in NetApp -Cloud-Volumes befindet, die über eine öffentliche Cloud-Plattform verfügbar sind.  Das Toolkit unterstützt zunächst die Google Cloud NetApp Volumes (GCNV), kann aber erweitert werden, um andere Cloud-Volumes wie Azure NetApp Files (ANF) oder Amazon FSx for ONTAP (FSxN) zu unterstützen.' 
---
...
[role="lead"]
Die Lösung bietet ein auf Ansible basierendes Automatisierungs-Toolkit zum Einrichten, Klonen und Aktualisieren einer von Oracle geklonten Very Large Database (VLDB) aus einer Sicherungskopie eines Oracle-Datenbank-Images, die sich in NetApp -Cloud-Volumes befindet, die über eine öffentliche Cloud-Plattform verfügbar sind.  Das Toolkit unterstützt zunächst die Google Cloud NetApp Volumes (GCNV), kann aber erweitert werden, um andere Cloud-Volumes wie Azure NetApp Files (ANF) oder Amazon FSx for ONTAP (FSxN) zu unterstützen.



== Zweck

Die Nutzung der Quick-Clone-Funktion (Minuten) von NetApp ONTAP -Volumes für Oracle VLDB aus einer serverinternen Datenbank-Image-Sicherungskopie in der öffentlichen Cloud dient mehreren Zwecken.  Es bietet eine nahezu in Echtzeit verfügbare Produktionsdatenbank in voller Größe für Berichts-, Entwicklungs- oder UAT-Anwendungsfälle.  Dies spart Speicherkosten, insbesondere wenn ein Thin Cloning der primären Datenbank-Backup-Volumes eine Option ist.  Mit diesem auf Ansible basierenden Automatisierungs-Toolkit können Benutzer geklonte Oracle-Datenbanken nach benutzerdefinierten Zeitplänen einrichten, klonen und aktualisieren, um ein optimiertes Lebenszyklusmanagement zu erreichen.  Das Toolkit ist für den Anwendungsfall von Oracle VLDB vorgesehen, das in der öffentlichen Cloud von Google bereitgestellt wird, und verwendet GCNV als serverinternen Datenbank-Backup-Speicher.
...
* Erstellen oder aktualisieren Sie eine geklonte Oracle-Datenbank aus einer Sicherungskopie des primären Datenbankimages mithilfe eines Ansible-Playbooks auf einem Scheduler.
...
NOTE: Der Einfachheit halber sollte der Oracle-Zielserver des Klons identisch zum primären Oracle-Server konfiguriert sein, beispielsweise hinsichtlich des Oracle-Software-Stacks und des Verzeichnislayouts für Oracle Home usw.
...
[%collapsible%open]
====
Das Toolkit enthält eine Hosts-Datei, die die Quell- und Ziel-Oracle-Hosts definiert, auf denen das Ansible-Playbook ausgeführt wird.  Normalerweise umfasst es den primären Datenbankserver-Host und den Ziel-Klon-Datenbankserver-Host.  Es folgt eine Beispieldatei.  Ein Hosteintrag enthält die IP-Adresse des Zielhosts sowie den SSH-Schlüssel für den Benutzerzugriff auf den Host, um den Klon- oder Aktualisierungsbefehl auszuführen.  Der Zugriff auf den Google Cloud NetApp Volumes -Speicher und dessen Verwaltung erfolgt über die gcloud-CLI.
...
NOTE: Für eine sicherere Automatisierungsbereitstellung kann Ansible Vault zum Verschlüsseln vertraulicher Informationen wie Passwörter, Zugriffstoken oder Schlüssel usw. verwendet werden. Die Lösung deckt nicht die Implementierung von Ansible Vault ab, ist jedoch in der Ansible-Dokumentation gut dokumentiert.  Bitte beachten Sielink:https://docs.ansible.com/ansible/latest/vault_guide/index.html["Schutz sensibler Daten mit Ansible Vault"^] für Details.
...
[%collapsible%open]
====
Hostvariablen werden im Verzeichnis host_vars mit dem Namen {{ host_name }}.yml definiert, das nur für den jeweiligen Host gilt.  Für diese Lösung wird nur die Ziel-Klon-DB-Host-Parameterdatei konfiguriert.  Die primären Oracle-DB-Parameter werden in der Datei „Global Vars“ konfiguriert.  Unten sehen Sie ein Beispiel für die Hostvariablendatei orac.yml des Ziel-Oracle-Klon-DB, die eine typische Konfiguration zeigt.
...
[%collapsible%open]
====
Auf dem geklonten Oracle-Zielserver sollte derselbe Oracle-Software-Stack installiert und gepatcht sein wie auf dem Oracle-Quellserver.  Die Variable $ORACLE_HOME sollte idealerweise mit der Quell-Oracle-Servereinstellung übereinstimmen.  Wenn sich die Zieleinstellung ORACLE_HOME von der Konfiguration des primären Oracle-Servers unterscheidet, erstellen Sie einen symbolischen Link, um die Unterschiede zu umgehen.

Wenn die primäre Datenbank mit ASM konfiguriert ist, kann die primäre Gruppe der Datendateien zur ASM-Gruppe gehören und dieselbe ASM-Gruppe mit derselben Gruppen-ID sollte zum Klonhost hinzugefügt werden, um Berechtigungsprobleme zu vermeiden.
...
Um zusätzliche Datenbanken zu klonen, erstellen Sie eine separate Datei „oracle_bkup_clone_n_gcnv.yml“ und „oracle_bkup_clone_n_gcnv.sh“.  Konfigurieren Sie die Ansible-Zielhosts, die globale Datei vars.yml und die Datei hostname.yml im Verzeichnis host_vars entsprechend.
